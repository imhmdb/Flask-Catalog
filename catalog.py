...
# Loading keys from client_secrets.json file
...
# gconnect and disconnect methods taken from the course
...
def showCategory(category_id):
    # shows the gategory items with the given id
...
def editItem(item_id):
    # shows the edit page for the given item
    item = session.query(Item).filter_by(id=item_id).first()
    if (item is None):
...
@app.route('/item/<int:item_id>/delete', methods=["get"])
